  ...
  >
    ...
    <template #navigation>
      <SfHeaderNavigationItem v-for="(x, i) in tree" :key="i">
        <nuxt-link data-cy="app-header-url_women" :to="localePath('/c/' + x.slug)">
          {{x.label}}
        </nuxt-link>
...
import { useCart, useWishlist, useUser, cartGetters, useCategory } from '~/composables';
...
export default {
  ...
  setup(props, { root }) {
    ...
    const { loadTree, loadingTree, tree } = useCategory();
